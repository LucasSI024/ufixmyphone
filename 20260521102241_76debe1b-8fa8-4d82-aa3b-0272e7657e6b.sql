...
import { Smartphone, ImagePlus, X, Euro, AlertTriangle, ShieldCheck } from "lucide-react";
...
function NewRequestPage() {
  ...
  const estimate = getMarketEstimate(form.problem_type);
  const budgetValue = form.budget_max ? Number(form.budget_max) : null;
  const budgetSignal = getBudgetSignal(form.problem_type, budgetValue);
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    const description = [
      ...
      `**Marktindicatie:** ${getMarketEstimate(form.problem_type).label}`,
        ...
        <fieldset className="space-y-4">
          ...
          <div className="rounded-2xl border border-primary/25 bg-primary/5 p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="font-semibold text-foreground">{estimate.label}</p>
                <p className="mt-1 text-xs text-muted-foreground">{estimate.note}</p>
                <p className="mt-2 text-xs text-muted-foreground">
                  Tip: laat het budget leeg als je vooral wilt weten wat een eerlijke marktprijs is. Een te laag maximum kan goede reparateurs afschrikken.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-2">
              <Label htmlFor="budget">Richtbudget (€) — optioneel</Label>
              <Input
                id="budget"
                type="number"
                min="0"
                step="1"
                placeholder="Laat leeg voor open biedingen"
                value={form.budget_max}
                onChange={upd("budget_max")}
              />
              <p className="text-xs text-muted-foreground">
                Dit is geen vaste prijsafspraak, maar helpt reparateurs inschatten of reageren zinvol is.
              </p>
            </div>
          </div>
